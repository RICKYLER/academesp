...
import StudyGroupDetailModal from './StudyGroupDetailModal';
...
const studyGroups = [
  {
    ...
    rating: 4.9,
    totalRatings: 15,
    nextMeeting: 'Tuesday, Dec 10 at 7:00 PM'
  },
  {
    id: 2,
    ...
    rating: 4.7,
    totalRatings: 22,
    nextMeeting: 'Wednesday, Dec 11 at 6:00 PM'
  },
  {
    id: 3,
    ...
    rating: 4.6,
    totalRatings: 8,
    nextMeeting: 'Monday, Dec 9 at 4:00 PM'
  },
  {
    id: 4,
    ...
    difficulty: 'Intermediate',
    isJoined: false,
    location: 'Physics Lab & Online',
    meetingType: 'hybrid' as const,
    tags: ['Physics', 'Mechanics', 'Thermodynamics'],
    instructor: 'Prof. David Kim',
    rating: 4.8,
    totalRatings: 18,
    nextMeeting: 'Friday, Dec 13 at 5:00 PM'
  }
];
...
const StudyGroupsContent: React.FC = () => {
  ...
  const [selectedGroup, setSelectedGroup] = useState<typeof studyGroups[0] | null>(null);
  ...
  const handleViewDetails = (group: typeof studyGroups[0]) => {
  ...
  const handleJoinGroup = async (groupId: number) => {
    ...
    if (selectedGroup?.id === groupId) {
      setSelectedGroup(prev => prev ? { ...prev, isJoined: true, members: prev.members + 1 } : null);
    }
  };
  ...
  const handleLeaveGroup = async (groupId: number) => {
    ...
    setGroups(prevGroups =>
      prevGroups.map(group =>
        group.id === groupId
          ? { ...group, isJoined: false, members: Math.max(group.members - 1, 0) }
          : group
      )
    );

    // Update selected group if it's the one being left
    if (selectedGroup?.id === groupId) {
      setSelectedGroup(prev => prev ? { ...prev, isJoined: false, members: Math.max(prev.members - 1, 0) } : null);
    }
  };
